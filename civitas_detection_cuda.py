...
if USE_CUDA:
    print(f"✅ CUDA enabled — {cv2.cuda.getCudaEnabledDeviceCount()} device(s) found")
    try:
        import cupy as cp
        USE_CUPY = True
        print("✅ CuPy available — color masking akan pakai GPU")
    except ImportError:
        USE_CUPY = False
        print("⚠️  CuPy tidak tersedia — color masking tetap CPU")
else:
    USE_CUPY = False
    print("⚠️  CUDA tidak tersedia — fallback ke CPU")
...
class Config:
    ...
    ORB_NFEATURES  = {'Dark': 800, 'Normal': 1000, 'Bright': 1300}
    ORB_ROI_SIZE   = {'Dark': 120, 'Normal': 160, 'Bright': 180}
    DETECT_EVERY_N = {'Dark': 1, 'Normal': 2, 'Bright': 1}
...
class CivitasDetector:
    def __init__(self):
        ...
        # CUDA filter untuk sharpening
        if USE_CUDA:
            try:
                self._cuda_filter = cv2.cuda.createLinearFilter(
                    cv2.CV_8UC1, cv2.CV_8UC1, self._sharpen_k
                )
                self._cuda_eq = True
            except Exception:
        ...
        for template_type, path in Config.UB_LOGO_TEMPLATES.items():
            if os.path.exists(path):
                ...
                for size in [80, 120, 160]:
                    ...
                    kp, des = self._detect_and_compute(resized)
                    if des is not None:
                        self.templates.append({"image": resized, "kp": kp, "des": des, "size": size})
                print(f"✓ Loaded ORB template: {template_type} (3 scales)")

    def _detect_and_compute(self, gray_img: np.ndarray):
        """ORB detectAndCompute — CUDA jika tersedia, fallback CPU."""
        if self._cuda_orb:
            try:
                gpu_img = cv2.cuda_GpuMat()
                gpu_img.upload(gray_img)
                gpu_kp, gpu_des = self.orb.detectAndComputeAsync(gpu_img, cv2.cuda_GpuMat())
                kp  = self.orb.convert(gpu_kp)
                des = gpu_des.download() if gpu_des is not None and not gpu_des.empty() else None
                return kp, des
            except Exception:
                pass
        return self.orb.detectAndCompute(gray_img, None) if not self._cuda_orb else \
               cv2.ORB_create(nfeatures=1000).detectAndCompute(gray_img, None)

    def _equalize_cuda(self, gray_img: np.ndarray) -> np.ndarray:
        """equalizeHist via CUDA jika tersedia."""
        if self._cuda_eq:
            try:
                gpu = cv2.cuda_GpuMat()
                gpu.upload(gray_img)
                out = cv2.cuda.equalizeHist(gpu)
                return out.download()
            except Exception:
                pass
        return cv2.equalizeHist(gray_img)

    def _sharpen_cuda(self, gray_img: np.ndarray) -> np.ndarray:
        """Sharpen via CUDA linear filter jika tersedia."""
        if self._cuda_filter is not None:
            try:
                gpu = cv2.cuda_GpuMat()
                gpu.upload(gray_img)
                out = self._cuda_filter.apply(gpu)
                return out.download()
            except Exception:
                pass
        return cv2.filter2D(gray_img, -1, self._sharpen_k)
    ...
    def detect_ub_logo(self, chest_roi_gray: np.ndarray, roi_size: int):
        ...
        for processed in variants:
            kp2, des2 = self._detect_and_compute(processed)
    ...
    def _color_ratio_cuda(self, hsv_cpu: np.ndarray, lower, upper) -> float:
        """cv2.inRange + countNonZero — CuPy jika tersedia, fallback CPU."""
...
class JetsonCivitasSystem:
    def __init__(self):
        ...
        if USE_CUDA:
            print(f"🚀 CUDA acceleration aktif")
            ...
            while True:
                ...
                if USE_CUDA:
                    try:
                        ...
                        self.brightness.update(gpu_gray)
                    except Exception:
                ...
                # Haar cascade every frame — no throttling (tetap CPU, lebih stabil)
